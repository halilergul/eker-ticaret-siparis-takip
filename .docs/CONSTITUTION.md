...
| 2026-06-20 | 011: Adapter-içi pagination loop (DRY refactor sonraya) | 4 panel'in pagination DOM/URL pattern'i farklı (Enderyapı SPA `?page=N`, Yedekler Classic ASP `?sayfa=N`, İkizler+Levent single-page). Rule-of-three yetersiz; yapay ortak helper soyutlanmıyor. Eğer 5+ adapter aynı pattern'i kullanırsa `lib/scraper/common/pagination.ts` çıkartılır. |
| 2026-06-20 | 011: `ScrapeContext.pagesVisited` opsiyonel telemetry + summary JSONB extensible alan | scrape_runs.summary JSONB olduğu için yeni `pages_visited` alanı şema migration gerektirmedi. Adapter `ctx.pagesVisited = N` yazar, writer summary'e geçirir. Eski runs için `null`/`undefined` (backwards-compatible). |
| 2026-06-20 | 011: Workflow timeout 8→30 dk (runner-minutes 15→35) | Enderyapı 172 sipariş pagination + 489 ürün catalog = 21-25 dk runner'da. Cron `daily_hour_utc` ile günde 1 koşum × 4 supplier × 30 dk = 3600 dk/ay teorik max ama 4 tedarikçi `schedule.enabled=false` (kullanıcı seçeneği) → şu an cron tetiklemiyor, free tier OK. Cron etkinleştirilirse 30dk timeout yeterli. |
| 2026-06-20 | 012: Zamlanan ürünler pencere kaldırıldı; karşılaştırma son sipariş anı baz | Eski `get_price_changes(window_days)` snapshot pencere bazlıydı; 1 yıl rafta + birden fazla zam gören ürünler kaçıyordu. Yeni `get_price_changes_v2`: `MAX(order_items.unit_price_at_order)` (son sipariş anı) vs `MAX(price_snapshots.unit_price)` (en güncel snapshot). Birikimli zam delta otomatik kapsanır. Hem KDV hariç olduğu için normalize yok (006/009/010 modeli). Snapshot eksik durumda satır listede kalır + UI rozet ("Bilinmiyor"). |
