/**
 * İkizler Hırdavat DOM keşif aracı — Feature 011 (pagination).
 *   npm run diag:ikizler -- --phase orders-pagination [--headed]
 * Output: tmp/ikizler-diag/<phase>/
 */
...
} from "@/lib/scraper/adapters/ikizler.constants";

type Phase = "login" | "orders-pagination" | "all";

const DIAG_ROOT = path.resolve(process.cwd(), "tmp/ikizler-diag");
...
async function dumpPage(page: Page, dir: string, label: string) {
  ...
  console.log(`  📄 ${dir}/${label}.html  🖼️  ${dir}/${label}.png`);
}
...
async function phaseOrdersPagination(page: Page) {
  ...
  const candidates = [
    ...
    "button:has-text('Sonraki')", "button:has-text('Next')",
    ".page-link", "[class*='paginat' i]",
  ];
  console.log("\n  Pagination DOM:");
  ...
  for (const q of ["?page=2", "?sayfa=2"]) {
    ...
    try {
      ...
      console.log(`  Sayfa 1 ile aynı mı: ${same ? "❌ aynı (cache)" : "✅ farklı (pagination ÇALIŞIYOR)"}`);
...
async function main() {
  ...
  console.log(`🚀 İkizler diag — phase: ${phase}`);
